...
def cell_number_to_position(number):
    pos_y = number % 16
    pos_x = int(number / 16)
    return pos_x, pos_y
...
def write_cell_walls(pos_x, pos_y, orientation):
    numbers[cell_position_to_number(pos_x, pos_y)] += 0
...
def is_in_the_center(pos_x, pos_y):
    return True if pos_x in (7, 8) and pos_y in (7, 8) else False
...
def get_cell_weight(pos_x, pos_y, orientation):
    ...
    return {
        "left": weight_on_left,
        "front": weight_on_front,
        "right": weight_on_right,
        "back": weight_on_back,
    }


def get_cell_neighbours_weight(pos_x, pos_y):
    weight_on_north = (
        numbers[cell_position_to_number(pos_x, pos_y + 1)] if pos_y != 15 else 1000
    )
    weight_on_west = (
        numbers[cell_position_to_number(pos_x - 1, pos_y)] if pos_x != 0 else 1000
    )
    weight_on_south = (
        numbers[cell_position_to_number(pos_x, pos_y - 1)] if pos_y != 0 else 1000
    )
    weight_on_east = (
        numbers[cell_position_to_number(pos_x + 1, pos_y)] if pos_x != 15 else 1000
    )

    return {
        "north": weight_on_north,
        "west": weight_on_west,
        "south": weight_on_south,
        "east": weight_on_east,
    }
...
for i in range(16):
    for j in range(16):
        x = i - 1
        y = j - 1
        numbers[cell_position_to_number(x, y)] = min(abs(x - 7), abs(x - 8)) + min(
            abs(y - 7), abs(y - 8)
        )

# numbers = list comprehension if I can
...
def get_self_weight(pos_x, pos_y):
    return numbers[cell_position_to_number(pos_x, pos_y)]


def are_there_walls(pos_x, pos_y):
    north = 0
    west = 0
    south = 0
    east = 0
    cell_byte = walls[cell_position_to_number(pos_x, pos_y)]
    if cell_byte >= 16:
        cell_byte -= 16
        north = 1
    if cell_byte >= 8:
...
def should_update_numbers(pos_x, pos_y, weights, are_there_walls):
    sorted_weights = sorted(weights.items(), key=operator.itemgetter(1))
    min_neighbour = 254
    for key, value in sorted_weights:
        if key == "back":
            if value < min_neighbour:
                min_neighbour = value
        else:
            if are_there_walls[key] == 0:
                if value < get_self_weight(pos_x, pos_y):
                    # the cell has a neighbour with lower weight
                    return 0
                else:
                    if value < min_neighbour:
                        min_neighbour = value
...
def should_update_numbers_dir_based(pos_x, pos_y):
    sorted_weights = sorted(
        get_cell_neighbours_weight(pos_x, pos_y).items(), key=operator.itemgetter(1)
    )
    are_there_walls_dir = are_there_walls(pos_x, pos_y)
    min_neighbour = 254
    for key, value in sorted_weights:
        if are_there_walls_dir[key] == 0:
            if value < get_self_weight(pos_x, pos_y):
                # the cell has a neighbour with lower weight
                return 0
            else:
